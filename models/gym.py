...
class AtariModel(nn.Module):
    def __init__(self, num_actions):
        ...
        self.actor = layer_init(nn.Linear(3136, num_actions), std=0.01)
...
class AtariICMModel(nn.Module):
    def __init__(self, num_actions):
        ...
        self.icm_inverse = layer_init(nn.Linear(1024, num_actions), std=0.01)
        self.icm_forward = nn.Sequential(
            layer_init(nn.Linear(512 + num_actions, 512)),
            nn.ReLU(),
            layer_init(nn.Linear(512, 512)),
            nn.ReLU(),
            layer_init(nn.Linear(512, 512)),
    ...
    def icm(self, s0, a, s1):
        ...
        a_onehot = F.one_hot(a, self.num_actions).float()
