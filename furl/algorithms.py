...
class PPOICMStrategy:
    ...
    def learn(self, model, memory, optimizer):
        ...
        state_feed = memory['state'][:-1].flatten(0, len(additional_dims)-1)
        next_state_feed = memory['state'][1:].flatten(0, len(additional_dims)-1)
        action_feed = memory['action'].flatten(0, len(additional_dims)-1)

        intrinsic_reward, inverse_loss = model.icm(state_feed, action_feed, next_state_feed)
        memory['intrinsic_reward'] = intrinsic_reward.reshape(*additional_dims)

        memory['reward'] = memory['intrinsic_reward']
